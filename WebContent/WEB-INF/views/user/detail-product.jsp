...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/asset/css/comment.css">
...
.rate label.nochecked {
background-image: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='126.729' height='126.73'%3e%3cpath fill='%23e3e3e3' d='M121.215 44.212l-34.899-3.3c-2.2-.2-4.101-1.6-5-3.7l-12.5-30.3c-2-5-9.101-5-11.101 0l-12.4 30.3c-.8 2.1-2.8 3.5-5 3.7l-34.9 3.3c-5.2.5-7.3 7-3.4 10.5l26.3 23.1c1.7 1.5 2.4 3.7 1.9 5.9l-7.9 32.399c-1.2 5.101 4.3 9.3 8.9 6.601l29.1-17.101c1.9-1.1 4.2-1.1 6.1 0l29.101 17.101c4.6 2.699 10.1-1.4 8.899-6.601l-7.8-32.399c-.5-2.2.2-4.4 1.9-5.9l26.3-23.1c3.8-3.5 1.6-10-3.6-10.5z'/%3e%3c/svg%3e");
}
.avatar::before {
	content: "";
	background-image: url("${pageContext.request.contextPath}/asset/images/avatar.png");
	width: 20px;
	height: 20px;
	margin-right: 10px;
}
...
<body>
	<div id="wrapper">
		...
		<div id="wp-content">
			<div class="container">
				...
				<div class="row">
					<div class="col-md-6 border-right img-product">
						<img
					...
					<div class="col-md-6">
						...
						<div class="p-2">
							<c:if test="${product.discount > 0 }">
								<span class="mr-4"><i class="fa fa-hand-holding-usd">
										Giá: </i> </span>
								<b><f:formatNumber minFractionDigits="0"
										value="${product.price *(100-product.discount) / 100}"
										type="number" />đ</b>
								<del style="font-style: italic;">
						...
						<hr
							style="border: 1px dotted rgba(0, 0, 0, .1); margin: 0px; margin-bottom: 10px">
						<form action="user/cart/addCart/${product.id}.htm" method="post"
							modelAttribute="cart">
							<label>Số Lượng</label> <input class="text-center" type="number"
								name="number" value="1" min="1" max="${product.in_stock}">
							<br>
							<div class="text-center mt-4">
								<c:if test="${product.in_stock > 0}">
									<button type="submit" class="btn btn-outline-danger"
										style="display: inline-block;">Thêm vào giỏ hàng</button>
									<a href="user/cart/addCart/${product.id}.htm" type="button"
										class="btn btn-lg btn-outline-danger" style="color: black">Mua
										Ngay</a>
								</c:if>
								<c:if test="${product.in_stock == 0}">
									<P>
										SẢN PHẨM HIỆN TẠI ĐANG HẾT HÀNG. LIÊN HỆ VỚI CHÚNG TÔI NẾU BẠN
										MUỐN ĐẶT HÀNG. <b>0338355872</b>
									</p>
								</c:if>
							</div>
				...
				<div id="comment" class="container" data-aos="flip-left">
					<div class="feedback">
						<p>Mời bạn chọn sao để đánh giá sản phẩm</p>
						<form:form action="user/comment/${product.id}.htm" modelAttribute="comment" >
							<div class="rating">
    ...
    <ellipse transform="scale(-1) rotate(45 454 -421.3)" cx="174.5"
		...
		<hr>
		<h4 class="container text-center">Đánh giá sản phẩm</h4>
		<div class="container list-rating">
			<c:forEach var="comment" items="${comments}">
				<div class="box-comment">
					<p  style="margin-bottom: 0.5rem;">
					<span>
						<img alt="" src="${pageContext.request.contextPath}/asset/images/avatar.png">
					</span>
					<span>${comment.userid.thongtinuser.fullname}</span>
					
					</p>
					<p><f:formatDate pattern="dd-MM-yyyy hh:mm:ss"
												value="${comment.utilTimestamp}" /></p>
					<c:choose>
						<c:when test="${comment.mark == 5 }">
							<div class="rate">
						...
						<c:when test="${comment.mark == 4 }">
							<div class="rate">
								<label class="checked"></label>
								<label class="checked"></label>
								<label class="checked"></label>
								<label class="checked"></label>
								<label class="nochecked"></label>
							</div>
						</c:when>
						<c:when test="${comment.mark == 3 }">
							<div class="rate">
								<label class="checked"></label>
								<label class="checked"></label>
								<label class="checked"></label>
	...
	<script type="text/javascript">
   		$(document).ready(function(){
   			...
   			if ('${message}') {
   				alert('${message}');
   				/* window.location.replace('https://localhost:8443/Web_BanHang/user/index.htm'); */
   				window.history.back(); 
   			}
   			if ('${message1}') {
   				alert('${message1}');
   			...
   			/* var mark = ${mark.mark};
   			if(mark == 5){
   				$('#rating-5').attr('checked', true);
   			}
   			else if(mark == 4){
   				$('#rating-4').attr('checked', true);
   			}
   			else if(mark == 3){
   				$('#rating-3').attr('checked', true);
   			}
   			else if(mark == 2){
   				$('#rating-2').attr('checked', true);
   			}
   			else {
